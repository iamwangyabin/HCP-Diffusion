...
class TextImagePairDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #     data = self.latents[img_name].copy()
            ...
            data = self.load_data(path, data_source, size)
            with h5py.File(self.latents, 'r') as file:
                data['img'] = torch.from_numpy(file[str(img_name)][...])
                # data = {'img':torch.from_numpy(file[str(img_name)][...]), 'crop_info':crop_info}
